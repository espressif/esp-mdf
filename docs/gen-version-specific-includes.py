...
# with version-based content for this MDF version
...
To obtain a local copy: open terminal, navigate to the directory you want to put ESP-MDF, and clone the repository using ``git clone`` command::
    ...
    cd ~/esp
    git clone %(clone_args)s--recursive https://github.com/espressif/esp-mdf.git
...
.. note::
    ...
    %(zipfile_note)s
"""
            ,"master" : 'This command will clone the master branch, which has the latest development ("bleeding edge") version of ESP-MDF. It is fully functional and updated on weekly basis with the most recent features and bugfixes.'
            ,"branch" : 'The ``git clone`` option ``-b %(clone_arg)s`` tells git to clone the %(ver_type)s in the ESP-MDF repository corresponding to this version of the documentation.'
            ,"zipfile" : {
                "stable" : 'As a fallback, it is also possible to download a zip file of this stable release from the `Releases page`_. Do not download the "Source code" zip file(s) generated automatically by GitHub, they do not work with ESP-MDF.'
                ,"unstable" : 'GitHub\'s "Download zip file" feature does not work with ESP-MDF.'
                },  # zipfile
            },  # git-clone
        "version-note" : {
            "master" : """
.. note::
     This is documentation for the master branch (latest version) of ESP-MDF. This version is under continual development. `Stable version`_ documentation is available, as well as other :doc:`/versions`.
"""
            ,"stable" : """
.. note::
     This is documentation for stable version %s of ESP-MDF. Other :doc:`/versions` are also available.
"""
            ,"branch" : """
.. note::
     This is documentation for %s ``%s`` of ESP-MDF. Other :doc:`/versions` are also available.
"""
            },  # version-note
    }, # en
    "zh_CN" : {
...
.. note::
    ...
    %(zipfile_note)s
"""
            ,"master" : '此命令将克隆 master 分支，该分支保存着 ESP-MDF 的最新版本，它功能齐全，每周都会更新一些新功能并修正一些错误。'
            ,"branch" : '``git clone`` 命令的 ``-b %(clone_arg)s`` 选项告诉 git 从 ESP-MDF 仓库中克隆与此版本的文档对应的分支。'
            ,"zipfile" : {
                "stable" : '作为备份，还可以从 `Releases page`_ 下载此稳定版本的 zip 文件。不要下载由 GitHub 自动生成的"源代码"的 zip 文件，它们不适用于 ESP-MDF。'
                ,"unstable" : 'GitHub 中"下载 zip 文档"的功能不适用于 ESP-MDF'
                },  # zipfile
            },  # git-clone
        "version-note" : {
            "master" : """
.. note::
     这是ESP-MDF master 分支（最新版本）的文档，该版本在持续开发中。还有 `Stable version`_ 的文档，以及其他版本的文档 :doc:`/versions` 供参考。
     This is documentation for the master branch (latest version) of ESP-MDF. This version is under continual development. `Stable version`_ documentation is available, as well as other :doc:`/versions`.
"""
            ,"stable" : """
.. note::
     这是ESP-MDF 稳定版本 %s 的文档，还有其他版本的文档 :doc:`/versions` 供参考。
"""
            ,"branch" : """
.. note::
     这是ESP-MDF %s ``%s`` 版本的文档，还有其他版本的文档 :doc:`/versions` 供参考。
"""
            },  # version-note
    }# zh_CN
}
