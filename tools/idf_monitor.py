    ...
    def _print(self, string, console_printer=None):
        ...
        if self._output_enabled:
            if self.print_timestamp:
                console_printer(bytes(f"[{datetime.datetime.now().isoformat(sep=' ', timespec='milliseconds')}] ",'utf-8'))
            console_printer(string)
        if self._log_file:
            try:
                if isinstance(string, type(u'')):
                    string = string.encode()
                string = bytes(f"[{datetime.datetime.now().isoformat(sep=' ', timespec='milliseconds')}] ",'utf-8') + string
...
def main():
    ...
    parser.add_argument(
        ...
        action='store',
        default=None,
        help="save log to file, the file name follow by this arg",
    )
    ...
    if args.save_log != None:
        yellow_print("save log mode to %s"%args.save_log)
        monitor.log_file_name = args.save_log
        monitor.toggle_logging()
    monitor.print_timestamp = args.timestamp
