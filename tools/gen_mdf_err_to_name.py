...
priority_headers = [ 'components/mcommon/include/mdf_err.h' ]
...
def process(line, mdf_path):
    """
    Process a line of text from file mdf_path (relative to IDF project).
    Fills the global list unproc_list and dictionaries err_dict, rev_err_dict
    """
    if mdf_path.endswith(".c"):
        # We would not try to include a C file
        raise InputError(mdf_path, "This line should be in a header file: %s" % line)

    words = re.split(r' +', line, 2)
    # words[1] is the error name
    # words[2] is the rest of the line (value, base + value, comment)
    if len(words) < 2:
        raise InputError(mdf_path, "Error at line %s" % line)
    ...
    if m:
        ...
        # Try to match a decimal number. Negative value is possible for some numbers, e.g. MDF_FAIL
        ...
        if re.match(r'@COMMENT@', line):
            ...
            for k in sorted(err_dict.keys()):
                ...
                for e in err_dict[k]:
                    ...
                    hexnum_length = 0
                    if k > 0: # negative number and zero should be only MDF_FAIL and MDF_OK
                        hexnum = " 0x%x" % k
...
def main():
    if 'MDF_PATH' in os.environ:
        mdf_path = os.environ['MDF_PATH']
    else:
        mdf_path = os.path.realpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))

    parser = argparse.ArgumentParser(description='ESP32 mdf_err_to_name lookup generator for mdf_err_t')
    parser.add_argument('--c_input', help='Path to the mdf_err_to_name.c.in template input.', default=mdf_path + '/components/mcommon/mdf_err_to_name.c.in')
    parser.add_argument('--c_output', help='Path to the mdf_err_to_name.c output.', default=mdf_path + '/components/mcommon/mdf_err_to_name.c')
    parser.add_argument('--rst_output', help='Generate .rst output and save it into this file')
    args = parser.parse_args()

    for root, dirnames, filenames in os.walk(mdf_path):
        for filename in fnmatch.filter(filenames, '*.[ch]'):
            full_path = os.path.join(root, filename)
            path_in_idf = os.path.relpath(full_path, mdf_path)
            if path_in_idf in ignore_files:
                continue
            with open(full_path, "r+") as f:
                for line in f:
                    # match also MDF_OK and MDF_FAIL because some of MDF_ERRs are referencing them
                    if re.match(r"\s*#define\s+(MDF_ERR_|MDF_OK|MDF_FAIL)", line):
                        try:
