...
#define QUICK_BLINK_FREQ            5//CONFIG_STATUS_LED_QUICK_BLINK_FREQ     /*!< default 5 */
#define SLOW_BLINK_FREQ             1//CONFIG_STATUS_LED_SLOW_BLINK_FREQ      /*!< default 1 */
#define LED_SPEED_MODE              LEDC_HIGH_SPEED_MODE//CONFIG_STATUS_LED_SPEED_MODE           /*!< default LEDC_HIGH_SPEED_MODE */

#define LED_QUICK_BLINK_CHANNEL     0//CONFIG_STATUS_LED_QUICK_BLINK_CHANNEL  /*!< default 0 */
#define LED_SLOW_BLINK_CHANNEL      1//CONFIG_STATUS_LED_SLOW_BLINK_CHANNEL   /*!< default 1 */
#define LED_NIGHT_MODE_CHANNEL      2//CONFIG_STATUS_LED_NIGHT_MODE_CHANNEL   /*!< default 2 */


#define LED_QUICK_BLINK_TIMER       0//CONFIG_STATUS_LED_QUICK_BLINK_TIMER    /*!< default 0 */
#define LED_SLOW_BLINK_TIMER        1//CONFIG_STATUS_LED_SLOW_BLINK_TIMER     /*!< default 1 */
#define LED_NORMAL_TIMER            2//CONFIG_STATUS_LED_NIGHT_MODE_TIMER     /*!< default 2 */
...
static esp_err_t led_level_set(led_handle_t led_handle, uint8_t level)
{
    ...
    if ((level & 0x01) ^ led_dev->dark_level) {
        if (led_dev->mode == LED_NIGHT_MODE) {
            ...
            gpio_set_level(led_dev->io_num, 0x1 ^ (led_dev->dark_level));
...
led_handle_t iot_led_create(uint8_t io_num, led_dark_level_t dark_level)
{
    ...
    gpio_cfg.pin_bit_mask = 1ULL << io_num;
...
esp_err_t iot_led_state_write(led_handle_t led_handle, led_status_t state)
{
    ...
    switch (state) {
        case LED_OFF:
            led_level_set(led_handle, led_dev->dark_level);
            break;

        case LED_ON:
            led_level_set(led_handle, 0x1 ^ (led_dev->dark_level));
            break;
