...
python esptool.py --chip esp32 --port /dev/ttyUSB$serial erase_flash
python esptool.py --chip esp32 --port /dev/ttyUSB$serial --baud 921600 --before default_reset --after hard_reset write_flash -z --flash_mode dio --flash_freq 40m --flash_size detect 0x1000 bootloader.bin 0x8000 partitions.bin 0xc0000 $PROJECT_NAME.bin
...
python idf_monitor.py -b 115200 -p /dev/$1 -sf "${log_file_name}__ttyUSB$start_serial.log" $PROJECT_NAME.elf
